/* Generated By:JavaCC: Do not edit this line. PumaParser.java */
package plp.puma.parser;

import plp.puma.Programa;
import plp.puma.comando.Atribuicao;
import plp.puma.comando.ChamadaMetodo;
import plp.puma.comando.ComDeclaracao;
import plp.puma.comando.Comando;
import plp.puma.comando.IO;
import plp.puma.comando.IfThenElse;
import plp.puma.comando.New;
import plp.puma.comando.Read;
import plp.puma.comando.Sequencial;
import plp.puma.comando.Skip;
import plp.puma.comando.Throw;
import plp.puma.comando.TryCatch;
import plp.puma.comando.While;
import plp.puma.comando.Write;
import plp.puma.declaracao.Declaracao;
import plp.puma.declaracao.classe.DecClasse;
import plp.puma.declaracao.classe.DecClasseComposta;
import plp.puma.declaracao.classe.DecClasseGeneric;
import plp.puma.declaracao.classe.DecClasseSimples;
import plp.puma.declaracao.classe.DecEnumSimples;
import plp.puma.declaracao.classe.ListaId;
import plp.puma.declaracao.procedimento.DecParametro;
import plp.puma.declaracao.procedimento.DecProcedimento;
import plp.puma.declaracao.procedimento.DecProcedimentoComposta;
import plp.puma.declaracao.procedimento.DecProcedimentoSimples;
import plp.puma.declaracao.procedimento.ListaDeclaracaoParametro;
import plp.puma.declaracao.variavel.CompostaDecVariavel;
import plp.puma.declaracao.variavel.DecVariavel;
import plp.puma.declaracao.variavel.DecVariavelEnum;
import plp.puma.declaracao.variavel.DecVariavelObjeto;
...
import plp.puma.expressao.binaria.ExpAnd;
import plp.puma.expressao.binaria.ExpConcat;
import plp.puma.expressao.binaria.ExpEquals;
import plp.puma.expressao.binaria.ExpOr;
import plp.puma.expressao.binaria.ExpSoma;
import plp.puma.expressao.binaria.ExpSub;
import plp.puma.expressao.leftExpression.AcessoAtributo;
import plp.puma.expressao.leftExpression.AcessoAtributoId;
import plp.puma.expressao.leftExpression.AcessoAtributoThis;
import plp.puma.expressao.leftExpression.Id;
import plp.puma.expressao.leftExpression.LeftExpression;
import plp.puma.expressao.unaria.ExpLength;
import plp.puma.expressao.unaria.ExpMenos;
import plp.puma.expressao.unaria.ExpNot;
...
import plp.puma.memoria.ContextoCompilacao;
...
public class PumaParser implements PumaParserConstants {
	...
	public static void main(String args[]) {
		...
		if (args.length == 0) {
			System.out
					.println("Puma PLP Parser Version 0.0.1:  Reading from standard input . . .");
			parser = new PumaParser(System.in);
		} else if (args.length == 1) {
			System.out
					.println("Puma PLP Parser Version 0.0.1:  Reading from file "
							+ args[0] + " . . .");
			try {
			...
			System.out
					.println("Puma PLP Parser Version 0.0.1:  Usage is one of:");
		...
		System.out.println("Puma PLP Parser Version 0.0.1:  Reading from file "
				+ FILE_CONTEXTO_INICIAL + " . . .");
		try {
			parserContextoInicial = new PumaParser(new java.io.FileInputStream(
					FILE_CONTEXTO_INICIAL));
		} catch (java.io.FileNotFoundException e) {
			System.out.println("Java Parser Version 1.0.2:  File "
					+ FILE_CONTEXTO_INICIAL + " not found.");
			return;
		}
		...
		try {
			...
			System.out
					.println("Puma PLP Parser Version 0.0.1:  Puma environment program parsed successfully.");

			// Contexto de compilação e execução são os mesmos para o parser do
			// ambiente inicial e do programa
			...
			if (programa.checaTipo(contextoCompilacao)) {
				ListaValor ambienteInicial = programa
						.executar(contextoExecucao);

				// Execução do programa
				programa = parser.processaEntrada();
				System.out
						.println("Puma PLP Parser Version 0.0.1:  Puma program parsed successfully.");
				if (programa.checaTipo(contextoCompilacao)) {
				...
				System.out
						.println("Erro de tipo na inicializa\u00e7\u00e3o do ambiente.");
			}
		} catch (Exception e) {
			System.out
					.println("Puma PLP Parser Version 0.0.1:  Encountered errors during parse.");
			e.printStackTrace();
		}
	}

	/**
	 * Método chamado com o intuito de processar a entrada fornecida ao parser
	 * gerando objeto do tipo Programa.
	 **/
	...
	/*
	 * A partir de agora, serão expostos através desse tipo de comentário as
	 * partes da BNF envolvidas no codigo mostrado em seguida.
	 */
	...
	/**
	 * 
	 * ComandoSimples ::= Skip | ComDeclaracao | While | IfThenElse | IO |
	 * ChamadaMetodo | New | Atribuicao | "("Comando")
	 **/
	...
	/**
	 * TryCatch ::= "try {" Comando "}catch(" id "){" Comando "}"
	 **/
	final public TryCatch PTryCatch() throws ParseException {
		TryCatch resposta;
		Tipo tipoExcecao;
		Id id;
		Comando comandoTry;
		...
		if (jj_2_12(2147483647)) {
			...
			jj_consume_token(RBRACE);
			resposta = new TryCatch(tipoExcecao, comandoTry, comandoCatch,
					comandoFinally, id);
			...
			comandoFinally = PComando();
			jj_consume_token(RBRACE);
			resposta = new TryCatch(null, comandoTry, new Skip(),
					comandoFinally, null);
		} else {
			switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk) {
			case TRY:
	...
	/**
	 * Throw ::= "throw" messagem
	 **/
	final public Throw PThrow() throws ParseException {
		Valor mensagem;
		jj_consume_token(THROW);
		mensagem = PValorString();
		{
			if (true)
				return new Throw(((ValorString) mensagem).valor());
		}
		throw new Error("Missing return statement in function");
	}

	/**
	 * IfThenElse ::= "if" Expressao "then" "{" Comando "}" | "if" Expressao
	 ...
	 * ... Id "," ListaId
	 **/
	final public ListaId PListaIdComposto() throws ParseException {
		Id id1;
		ListaId id2;
		id1 = PId();
		jj_consume_token(COMMA);
	...
	/**
	 * ExpBinaria ::= ExpUnaria ( "+" ExpUnaria | "-" ExpUnaria | "and"
	 * ExpUnaria | "or" ExpUnaria | "==" ExpUnaria | "++" Expressao)*
	 **/
	...
	/**
	 * Uma expressao chamadora eh aquela que chama um método. Ela sabe separar o
	 * acesso de atributo, que é o que ela retorna, da chamada do método em si
	...
	/**
	 * LeftExpressionChamadora ::= AcessoAtributoChamador | AcessoAtributoThis |
	 * id
	 **/
	final public LeftExpression PLeftExpressionChamadora()
	 ...
	 **/
	final public AcessoAtributoId PAcessoAtributoIdChamador()
			throws ParseException {
		LeftExpression av;
		Id id;
	...
	final public DecClasse PDecClasseAtomica() throws ParseException {
		...
		if (tipoGenerico == null) {
			...
			classe = new DecClasseGeneric(nomeClasse, atributos, metodos,
					tipoGenerico);
		}
		{
			if (true)
	...
	/**
	 * ... "enum" Id "{" DecVariavel ";" DecProcedimento "}"
	 **/
	final public DecEnumSimples PDecEnumAtomica() throws ParseException {
		Id nomeEnum;
		ListaId ids;
		DecVariavel atributos = null;
		DecProcedimento metodos = null;
		...
		{
			if (true)
				return new DecEnumSimples(nomeEnum, ids, atributos, metodos);
		}
		throw new Error("Missing return statement in function");
	}
	 ...
	 **/
	final public CompostaDecVariavel PCompostaDecVariavel()
			throws ParseException {
		DecVariavel d1;
		DecVariavel d2;
	 ...
	 * ... Tipo Id ":=" Tipo
	 **/
	final public DecVariavelEnum PDecVariavelEnum() throws ParseException {
	...
	/**
	 * DecProcedimento ::= "proc" Id "("")" "{"Comando"}" | "proc" Id "("
	 * ListaDeclaracaoParametro ")" "{"Comando"}" | DecProcedimento ","
	 * DecProcedimento
	 **/
	 ...
	 **/
	final public DecProcedimentoComposta PDecProcedimentoComposta()
			throws ParseException {
		DecProcedimento d1;
		DecProcedimento d2;
	...
	/**
	 * ... "proc" Id "("")" "{"Comando"}"|"proc" Id "(" ListaDeclaracaoParametro
	 * ")" "{"Comando"}"
	 **/
	final public DecProcedimento PDecProcedimentoSimples()
			throws ParseException {
		Id nome;
	 ...
	 **/
	final public ListaDeclaracaoParametro PListaDeclaracaoParametro()
			throws ParseException {
	 ...
	 **/
	final public ListaDeclaracaoParametro PDecParametroComposto()
			throws ParseException {
		DecParametro d1;
	 ...
	 **/
	final public Tipo PTipoConcreto() throws ParseException {
		Tipo tipo;
	 ...
	 * TipoClasse<TipoConcreto> | TipoClasse<TipoGeneric>
	 **/
	final public Tipo PTipoClasseGeneric() throws ParseException {
		Id idTipoClasse;
		Tipo tipoGeneric;
		idTipoClasse = PId();
		jj_consume_token(LT);
		tipoGeneric = PTipoGeneric();
		jj_consume_token(GT);
		{
			if (true)
	 ...
	 * TipoGeneric ::= <Id>
	 **/
	final public Tipo PTipoGeneric() throws ParseException {
		Id tipoGeneric;
		tipoGeneric = PId();
		{
			if (true)
	...
	private static void jj_la1_init_0() {
		jj_la1_0 = new int[] { 0xc23b0000, 0x0, 0x80000000, 0x200000,
				0x2000000, 0xa007800, 0x0, 0x80000, 0x8006000, 0x0, 0x2000,
				0xa007800, 0x600, 0x600, 0x0, 0x2000000, 0x0, 0x20000000, 0x0,
				0x20000000, 0x0, 0x10000000, 0x10000000, 0x0, 0x0, 0x0, 0x0, };
	}

	private static void jj_la1_init_1() {
		jj_la1_1 = new int[] { 0x14800, 0x4000, 0x0, 0x0, 0x4800, 0x4c40,
				0x800, 0x0, 0x440, 0x400, 0x0, 0x4c40, 0x0, 0x0, 0x4800, 0x0,
				0x800, 0x0, 0x8000000, 0x0, 0x8004838, 0x0, 0x0, 0x8000838,
				0x838, 0x800, 0x20, };
	}

	private static void jj_la1_init_2() {
		jj_la1_2 = new int[] { 0x0, 0x0, 0x0, 0x0, 0x0, 0x100, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x100, 0x1c1, 0x1c1, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
	...
	public PumaParser(java.io.InputStream stream) {
		this(stream, null);
	}

	/** Constructor with InputStream and supplied encoding */
	public PumaParser(java.io.InputStream stream, String encoding) {
		try {
			jj_input_stream = new JavaCharStream(stream, encoding, 1, 1);
		} catch (java.io.UnsupportedEncodingException e) {
			throw new RuntimeException(e);
		}
		token_source = new PumaParserTokenManager(jj_input_stream);
		token = new Token();
		jj_ntk = -1;
		jj_gen = 0;
		for (int i = 0; i < 27; i++)
	...
	/** Constructor. */
	public PumaParser(java.io.Reader stream) {
	...
	public PumaParser(PumaParserTokenManager tm) {
		token_source = tm;
		token = new Token();
		jj_ntk = -1;
		jj_gen = 0;
	...
	/** Reinitialise. */
	public void ReInit(PumaParserTokenManager tm) {
		token_source = tm;
		token = new Token();
		jj_ntk = -1;
		jj_gen = 0;
